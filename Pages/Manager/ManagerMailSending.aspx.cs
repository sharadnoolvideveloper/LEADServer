...
public partial class Pages_Manager_ManagerMailSending : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                rptStudentDetail.DataSource = BLobj.Manager_FillStudentsCollegeWiseForSMSMail(cook.Manager_Id(), "", ddlStudentType.SelectedValue.ToString(), ddlProjectStatus.SelectedValue.ToString(), ddlAcademicYear.SelectedValue.ToString());
    ...
    protected void btnMailMain_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            rptStudentDetail.DataSource = BLobj.Manager_FillStudentsCollegeWiseForSMSMail(cook.Manager_Id(), strCollegeCode.ToString(), ddlStudentType.SelectedValue.ToString(), ddlProjectStatus.SelectedValue.ToString(), ddlAcademicYear.SelectedValue.ToString());
            rptStudentDetail.DataBind();
            lblTotalStudentsCount.Text = rptStudentDetail.Items.Count.ToString();
    ...
    protected void btnSendMail_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                Subject = cook.ManagerName() + " " + "Wants to Share some Imfomation" + " " + txtSubject.Text.Trim(),
    ...
    protected void btnSendSMS_Click(object sender, EventArgs e)
    {
        ...
        {
            BLobj.SendMailException("btnSendSMS_Click", ex.ToString(), "ManagerMailSending.aspx", cook.Manager_Id(), "");
        }
    }
    ...
    protected void BtnSendNotification_Click(object sender, EventArgs e)
    {
            ...
            else
            {
                foreach (RepeaterItem ri in rptStudentDetail.Items)
                {
                    ...
                    if (ChkStudentSelect.Checked == true)
                    {
                        ...
                        string ServerResponse = FCMPushNotification.AndroidPush(DeviceId.ToString(), txtSMSMessage.Text.ToString(), "Student", "Empty");
                        BLobj.Manager_SaveNotificationLog(cook.Manager_Id(), lblLeadId.Text.ToString(), txtSMSMessage.Text.ToString(), "Notification", ServerResponse.ToString());
                        if (i == 0)
                        {
                            string Subject = "Notification" + "-" + cook.ManagerName() + " " + "send Notification to Students";
        ...
        {
            BLobj.SendMailException("BtnSendNotification_Click", ex.ToString(), "ManagerMailSending.aspx Notification", cook.Manager_Id(), "");
        }
    }
        ...
        try
        {
            foreach (RepeaterItem ri in rptStudentDetail.Items)
            {
                ...
                if (ChkStudentSelect.Checked == true)
                {
                    ...
                    BLobj.Manager_SaveNotificationLog(cook.Manager_Id(), lblLeadId.Text.ToString(), txtSMSMessage.Text.ToString(), "SMS", ServerResponse.ToString());
                    if (i == 0)
                    {
