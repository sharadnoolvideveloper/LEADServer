<%@ WebHandler Language="C#" Class="Get_NotificationTopCount" %>
...
public class Get_NotificationTopCount : IHttpHandler
{
    ...
    private string list()
    {
        ...
        try
        {
            ...
            string gstrQrystr = "select Type as Notification_Type,Message,Date_format(createDate,'%d-%m-%Y %h:%i:%s') as createDate from notification_log Where Lead_Id='"+cook.LeadId()+"' and status=0 order by Date_format(createDate,'%Y-%m-%d %h:%i:%s') desc";
